...
public class PushInfoDaoImpl implements IPushInfoDao {
	...
	public Map<String, Object> selectWarnWeek() throws SQLException {
		...
		String sqlWarnTop5 = "SELECT a.event_name,b.num FROM mep_event a INNER JOIN  (SELECT event_code,COUNT(1) AS num FROM mep_warning WHERE YEARWEEK(DATE_FORMAT(create_time,'%Y-%m-%d')) = YEARWEEK(NOW()) GROUP BY event_code ORDER BY num LIMIT 0,5) b ON a.event_code = b.event_code";
		DruidPooledConnection conn = instance.getConnection();
