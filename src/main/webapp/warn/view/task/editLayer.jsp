					...
					<form id="tableForm">
						...
						<div class="field-box">
							<label>*任务编码:</label> <input class="span5 inline-input"
								type="text" name="taskCode" value="${data.taskCode }"  readonly />
						</div>
						<div class="field-box">
							<label>*任务名称:</label> <input class="span5 inline-input"
								type="text" name="taskName"  value="${data.taskName }" placeholder="请输入任务名称..." />
						</div>
						<div class="field-box">
							<label>*任务类型</label>
							<div class="ui-select">
								<select id="taskType" name="taskType">
									...
									<option value="data" <c:if test="${data.taskType eq 'data' }">selected="selected"</c:if>>数据</option>
									<option value="work" <c:if test="${data.taskType eq 'work' }">selected="selected"</c:if>>业务</option>
								</select>
							</div>
						</div>
						<div class="field-box">
							...
							<div class="ui-select">
								<select id="taskRunType" name="taskRunType">
									...
									<option value="once" <c:if test="${data.taskRunType eq 'once' }">selected="selected"</c:if> >每日一次</option>
									<option value="cont" <c:if test="${data.taskRunType eq 'cont' }">selected="selected"</c:if>>每日循环</option>
								</select>
							</div>
						</div>
						<div class="field-box" id = "taskTime">
							<label>*任务执行时间间隔（单位：秒）</label> <input class="span5 inline-input"
								type="text" name="taskTime" value="${data.taskTime }" placeholder="请输入任务执行时间间隔..." />
						</div>
						<div class="field-box" id = "taskTiming">
							<label>*任务执行时间点</label> <input class="span5 inline-input form_datetime"
								type="text" name="taskTiming" value="${data.taskTiming }" placeholder="请输入任务执行时间点..." readonly />
						</div>
						<div class="field-box">
							<label>*任务状态:</label> <label style="width: 20%;"><input
								type="radio" name="status" value="Y" <c:if test="${data.status eq 'Y' }">checked="checked"</c:if>  />启用</label>
							<label style="width: 20%; float: left;"><input
								type="radio" name="status" value="N" <c:if test="${data.status eq 'N' }">checked="checked"</c:if> />未启用</label>
						</div>
		...
		$("#sumbit_form").on("click",function(){
			if(check()){
				$("#tableForm").ajaxSubmit({
					url:'taskManage?method=task_upt',
					type:'post',
					dataType:'json',
