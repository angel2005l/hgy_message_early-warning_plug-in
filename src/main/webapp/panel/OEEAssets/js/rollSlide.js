...
(function($) {
	$.fn.rollSlide = function(obj) {
		var $self = this, orientation = obj.orientation || 'left', // 滚动方式
		num = obj.num || 1, // 滚动数量
		v = (typeof obj.v === 'number') ? obj.v : 0, // 滚动速度
		minTime = (typeof obj.space === 'number') ? ((obj.space >= 100) ? obj.space
				: 100)
				: 100, // 最小间隔为 100 ms ，
		space = minTime + v || 5000 + v, // 滚动间隔 默认 5000ms
		isRoll = obj.isRoll, // 自动播放
		isStart = true, roll = function(ori, n, v) {
			var $ul = $self.find('.roll_list'), $item = $ul.find('li'), range = 0, i, len = $item.length, sliceItem = [], cloneSliceItem = [], startTime = (new Date())
					.getTime(),
			// 存放滚动过的 item
			memory = function() {
				var arr = [];
				...
				if (ori === 'left' || ori === 'top') {
					for (i = 0; i < n; i++) {
						range += ori === 'left' ? $($item[i]).outerWidth(true)
								: $($item[i]).outerHeight(true); // left 取
						// width，top
						// 取 height
						arr.push($item[i]);
					}
					...
					for (i = len - n; n > 0; n--, i++) {
						range += ori === 'right' ? $($item[i]).outerWidth(true)
								: $($item[i]).outerHeight(true);
						arr.push($item[i]);
					}
				}
				return arr;
			...
			var $ul = $self.find('.roll_list'), $item = $ul.find('li'), len = $item.length, timer;
	...
	$.fn.rollNoInterval = function() {
		var $self = this, $ul = $self.find('.roll_list'), $item = $ul
				.find('li'), len = $item.length, timer, top = function() {
			$item = $ul.find('li');
			...
			timer = setInterval( function() {
				...
				if (offset > -range) {
					...
					range = $sliceItem.outerHeight(false);
				}
			}, 50);
//			clearInterval(timer);
		}, init = function() {
